...
benbakermo = "793963034"
tweet_text = "@benbakermo Boo! #dragqeensrule #benbakerisabigot #moleg #benbakerdoesntcareaboutrealissues"
#tweet_text = "Ben, care about real issues, please?!?!"

response = client.get_users_tweets(id="793963034", max_results=5)

benbakermo_last_tweet_id = response.data[0].id
benbakermo_last_tweet_text = response.data[0].text

print(benbakermo_last_tweet_id)
print(benbakermo_last_tweet_text)

if not benbakermo_last_tweet_text.startswith("RT"):
    ...
    client.create_tweet(text=tweet_text, in_reply_to_tweet_id=benbakermo_last_tweet_id, media_ids=[media.media_id])
    print("Tweeted!")
else:
